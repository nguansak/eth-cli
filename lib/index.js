...
class EthereumWallet {
	...
	async setProvider(provider = false) {
		...
		else if (provider instanceof Web3.providers.HttpProvider || provider instanceof Web3.providers.WebsocketProvider || provider instanceof SignerProvider) {
	...
	async createRawTx(from, to, value, gasPrice = undefined, gasLimit = undefined) {
		...
		return lightwallet.txutils.valueTx({ value, to, gasPrice, nonce, gasLimit })
	}

	async createRawDataTx(from, data, value, gasPrice = undefined, gasLimit = undefined) {
		const nonce = await this.getNonce(from)
		const to = data.to.substr(2).toLowerCase()

		if (!gasPrice) gasPrice = await this.gasPrice
		if (!gasLimit) gasLimit = await this.estimateGas({ value, to, gasPrice, nonce, ...data })

		return lightwallet.txutils.valueTx({ value, gasPrice, nonce, gasLimit, ...data })
	}
	...
	async sendToAddress(from, to, value, gasPrice = undefined, gasLimit = undefined) {
		let rawtx = await this.createRawTx(from, to, value, gasPrice, gasLimit)
